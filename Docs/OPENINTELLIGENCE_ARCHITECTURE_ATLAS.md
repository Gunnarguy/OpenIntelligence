...
- **Local Model RAM Gate**: The local 20B model (AFM 3 Core Advanced) requires 12GB of RAM. The app implements programmatic physical memory checking (`physicalMemory >= 11.5GB` in `DeviceHardware`) to hide the 20B option and fallback to the 3B Core model on 8GB physical devices (iPhone 16 Pro Max), ensuring logs and UI state accurately represent hardware capability. `[evidence: code_verified, exact, LLMModel.swift, FoundationModelRoutePolicy.swift]`
