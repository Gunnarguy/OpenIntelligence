...
| PCC access                    | **NONE**         | Framework docs | Third-party apps get on-device only. No server model fallback.        |
| Server context (we DON'T get) | ~65K tokens      | Tech Report    | This exists but is Apple-internal only                                |
...
| ----------------------------- | -------------------------------------------------------------- | ----------------------------------------------- |
| GraphRAG entity resolution    | 3B model at 2-bit can't reliably disambiguate entities via LLM | NLTagger NER + PascalCase heuristics            |
| LLM-powered triple extraction | Needs large context + high reasoning quality                   | Rule-based entity co-occurrence                 |
| Community summarization       | Requires processing entire graph neighborhoods                 | RAPTOR-lite hierarchical summaries              |
...
| Three-index synchronization     | Text + vector + graph atomic updates          | We have text + vector (no graph layer)          |
...
- [ ] Does it work on-device only? (no PCC, no cloud dependency)
