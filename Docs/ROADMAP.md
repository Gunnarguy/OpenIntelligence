...
> **Documentation status:** Verified for OpenIntelligence v4.5.1 on 2026-07-04.
...
- **App Store Small Business Program - PCC Entitlement (v4.6):** Await approval for the App Store Small Business Program to acquire the `com.apple.developer.private-cloud-compute` entitlement. Once granted, re-enable the entitlement in `OpenIntelligence.entitlements` to unlock the server-side Private Cloud Compute foundation model and stop `EntitlementChecker` from forcing on-device model fallbacks. (As of v4.6 the checker fails closed — unprovable entitlement always routes on-device; revisit the check when the grant lands, since App Store builds strip the provisioning profile.)
