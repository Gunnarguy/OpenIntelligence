...
- **App Store Small Business Program - PCC Entitlement (v4.6):** Await approval for the App Store Small Business Program to acquire the `com.apple.developer.private-cloud-compute` entitlement. Once granted, re-enable the entitlement in `OpenIntelligence.entitlements` to unlock the 20B server-side Private Cloud Compute foundation model and stop `EntitlementChecker` from forcing 3B on-device model fallbacks.
