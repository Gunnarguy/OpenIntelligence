...
- Core AI embeddings are used in production, and **the model they run is `sentence-transformers/all-MiniLM-L6-v2`, not an Apple-authored embedder**. `THIRD_PARTY_NOTICES.md` binds the bundled `EmbeddingModel.mlpackage` to that model, which is the strongest provenance in this repository because the attribution is legally required. Core AI is an Apple framework and the zero-copy Silicon-native execution is real; the weights are third-party and 384-dimensional. The two provider options in Library Settings are therefore the same weights on two runtimes, so choosing between them changes latency and not accuracy. Runs on iOS 27+ / macOS 27+ compatible devices, falling back to Core ML. The compiled model is bundled inside a custom raw resource folder (`EmbeddingModel.bundle`) to bypass Xcode build-time `mlassetc` compiler checks that block minimum deployment targets below 27.0. `[evidence: code_verified, exact, CoreAISentenceEmbeddingProvider.swift, Package.swift]`
...
- Embedding input was truncated at 128 tokens until 2026-08-17 while both compiled models declare input shape `[1, 512]`. Measured with the real WordPiece vocab over 139 live chunks, median chunk is 273 tokens and **55% of library content never reached the embedder**. The bundled tokenizers also carried a `padding` block, which made `DocumentProcessor.countTokens` return a constant (logged `maxTokens=128/430` on 3,910 of 3,910 ingestions) and caused mean pooling to average `[PAD]` into every vector. Fixed in `2753d15` by removing the padding block and raising truncation to 512. **Vectors created before that date remain truncated and padding-diluted until their library is re-embedded, and nothing in the app detects this.** Sequence length and embedding dimension are independent: dimensionality is unchanged at 384 and `BNNSVectorDatabase`'s format is untouched. `[evidence_level: measured, confidence: high, evidence_source: real tokenization of 139 live chunks; INGESTION_PIPELINE.md section 5b]`
...
## 18. App Icon Appearance

- The AppIcon asset catalog retains the existing light mark and includes a universal iOS dark luminosity rendition. The compiled asset catalog contains a `UIAppearanceDark` AppIcon entry; physical-device selection remains unverified. `[evidence_level: build_verified, confidence: high_pending_device_validation, evidence_source: OpenIntelligence/Resources/Assets/Assets.xcassets/AppIcon.appiconset/Contents.json; xcrun actool; /usr/bin/assetutil --info]`
