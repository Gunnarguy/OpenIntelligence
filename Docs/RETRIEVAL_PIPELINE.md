...
17. **Retrieval has a second consumer, and it needs the passage judged rather than the output judged** *(added 2026-08-15)*: `SuggestedQuestionsService` runs the same retrieval and turns the chunks into the questions offered before the user types anything. `buildGroundedPassages` took `chunks.prefix(limit)` with no quality test, so whatever retrieval returned became question material.
    ...
    `questionSourcePenalty` scores the chunk instead, on citation density, dot leaders, access boilerplate, digit-to-letter ratio, vowel-free token ratio, and absence of sentence enders. It **demotes rather than filters**, because a hard filter returns an empty set for a document that is entirely a reference list or a scanned table, which disables the feature precisely where it is already weakest. `[evidence_level: measured, confidence: high, evidence_source: the function extracted verbatim and run against nine samples: bibliography 5, numbered references 5, table of contents 8, running header 4, numeric table 4, and 0 for body prose, prose citing a work, a lease contract, and prose containing many numbers]`
