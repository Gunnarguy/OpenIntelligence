...
| **Model Routing** | Hardcoded heuristics; pre-retrieval PCC prediction. | **Post-retrieval `ModelExecutionPlanner` + `ModelExecutionReceipt`** using evidence, exact/labeled budgets, signed entitlement, fail-closed quota handling, consent, minimized envelopes, and local verification. | Device evaluation and data-driven policy tuning; Apple Dynamic Profiles are not integrated and remain deferred. | **Source complete; device validation pending** |
