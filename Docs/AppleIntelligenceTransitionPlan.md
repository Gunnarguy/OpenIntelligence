...
- **Dark-mode AppIcon appearance (Implemented 2026-08-23):** The existing light icon remains the default and the asset catalog now supplies a universal iOS dark luminosity rendition for device appearance selection. `[evidence_level: build_verified, confidence: high_pending_device_validation, evidence_source: AppIcon.appiconset/Contents.json; xcrun actool; assetutil UIAppearanceDark rendition]`
