...
> **Addendum, same day — `665da0a` fixed Deep Think only.** It threaded chunks through `executeReasoningChain`, which Maximum does not use. Maximum runs `executeTrueUnlimitedReasoning` and `executeMultiChainReasoning` and kept the original defect verbatim: a device trace retrieved 103 chunks at 84/72/67% rerank scores, logged `Integrating 0 insights into comprehensive answer`, then `Model unavailable (inference failed)`, and returned a 232-character non-answer in 17.8s. Fixed in `b271ecf` by threading all five remaining sites in Maximum's path plus `getResearchDecision`, which passed `context: ""` with no chunks and so abstained on the recursive-research loop's own decision step. An audit of all fourteen `generateWithProperConsent` call sites now holds the invariant that **no site passes both an empty context and no chunks**, which is what makes this class of defect unable to recur silently. Lesson worth keeping: fixing the function the failing trace pointed at is not the same as fixing the defect, and only the call-site audit proved the difference.
