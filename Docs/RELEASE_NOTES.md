> **Documentation status:** Source-verified and simulator-compiled for OpenIntelligence v4.6 on July 15, 2026. All 8 PCC routing tests pass; the full suite is 106/107 due to an independent SemanticChunker named-entity expectation. Signed physical-device and distribution validation remains pending.
...
*   **Privacy-Safe Consent and Fallbacks:** PCC consent now occurs only after the cloud payload is final. The sheet shows context size, bytes, and route reason; background and App Intent work never waits for foreground UI. Automatic/prefer-cloud may fall back locally before streaming, cloud-only fails explicitly, quota is not retried on PCC, and partial cloud/local streams are never combined. `[evidence_level: code_verified, confidence: high, evidence_source: CloudConsentPromptView.swift, RAGService.swift, AgenticOrchestrator.swift]`
*   **Managed Entitlement Enabled:** Apple approval was confirmed on July 15 and the source entitlement now includes `com.apple.developer.private-cloud-compute`. Runtime proof reads the signed process entitlement, while the native session factory rechecks availability and quota immediately before construction. This is source-complete, not yet signed-device/TestFlight verified. `[evidence_level: code_verified+user_confirmed, confidence: high_for_source_unverified_for_distribution, evidence_source: OpenIntelligence.entitlements, EngineSDKCompatibility.swift, FoundationModelSessionFactory.swift]`
...
*   **PCC Entitlement Fail-Closed (superseded implementation):** `EntitlementChecker.hasEntitlement` now reads the signed process entitlement through Security.framework instead of depending on an embedded provisioning profile, which distribution may strip. Missing proof remains a fail-closed result.
