...
- **You can write your own reasoning profile.** Apple's control takes three fixed settings or a description you write yourself, and the app had only ever used the fixed three. In Model Parameters you can now write something like "work backwards from the conclusion and say which step is weakest" and it replaces the setting for questions that reason. It only affects answers written on Private Cloud Compute, because reasoning does not happen on the device at all, and the field says so rather than letting you write one and wonder why nothing changed.
