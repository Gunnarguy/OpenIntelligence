...
### Getting Started
- **The welcome screens follow your light or dark setting now.** They were always dark, whatever the rest of the app was doing, so turning the phone to light mode gave you a white app with one navy screen at the front of it. In light mode the status bar was the real casualty: iOS draws the clock and battery in dark ink on a light screen, and the welcome screen underneath was nearly black, so they simply vanished.
- **Light is its own design, not the dark one flipped.** Inverting would have given a flat pale rectangle. The light version is a warm off-white settling into a faint blue-grey, with the glow behind it eased back, so it still feels like a lit surface rather than a blank page.
...
### Answering
- **Three sliders in Model Parameters were telling you they did something they cannot do.** Frequency Penalty, Presence Penalty and Repetition Penalty sat under a line saying they affect how the model picks words. Apple's generation API has no penalty setting of any kind, so Apple Intelligence ignores all three, on this device and on Private Cloud Compute. They now say so. They are still there, and still adjustable, because they are for a local model host on Mac that is not built yet.
...
- **It works in Deep Think and Maximum now too.** When it first shipped it only applied to Standard, which was most of the point missed: those are the modes where how carefully a question gets answered matters most. They use the temperature it picks and keep their own per-step lengths, because those run many passes with budgets chosen for what each pass is doing.
- **You can write your own reasoning profile.** Apple's control takes three fixed settings or a description you write yourself, and the app had only ever used the fixed three. In Model Parameters you can now write something like "work backwards from the conclusion and say which step is weakest" and it replaces the setting for questions that reason. This is separate from the app's own reasoning: Deep Think and Maximum think across many steps on your device and that is unaffected by what you write here. What this changes is how hard Apple's model works inside a single answer, which Apple only allows on Private Cloud Compute.
