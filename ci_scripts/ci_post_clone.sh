...
if [ "$UNRELEASED_ENTRIES" -gt 0 ]; then
    echo "======================================================================"
    echo "BUILD STOPPED: CHANGELOG.md may name a version that already shipped"
    echo "======================================================================"
    echo "File read:          $CHANGELOG"
    echo "Heading read:       the first line matching '^## [0-9]', which begins"
    echo "                    '## $LATEST_VERSION'"
    echo "Version extracted:  $LATEST_VERSION (stamped as MARKETING_VERSION for iOS and macOS)"
    echo ""
    echo "Why that is a problem: '## [Unreleased]' holds $UNRELEASED_ENTRIES entry line(s)."
    echo "Work in progress belongs under an open '## X.Y <!-- unreleased -->'"
    echo "heading, so entries under [Unreleased] mean the next version was never"
    echo "opened and $LATEST_VERSION has most likely shipped already. App Store"
    echo "Connect rejects a build whose version is already released; that is how"
    echo "the 2026-07-28 build failed."
    echo ""
    echo "Fix, one line, before the next source push: at the top of CHANGELOG.md,"
    echo "directly under '## [Unreleased]', open the next version:"
    echo ""
    echo "    ## X.Y <!-- unreleased -->"
    echo ""
    echo "with X.Y above $LATEST_VERSION. The entries then sit under X.Y, [Unreleased]"
    echo "is empty again, and the next build stamps X.Y. If $LATEST_VERSION has not"
    echo "shipped, move the entries under '## $LATEST_VERSION' instead."
    echo "======================================================================"
    exit 1
fi
