...
namespace mtmd
{
    ...
    struct bitmap
    {
        ...
        bitmap(const bitmap &other) : ptr(clone(other.ptr.get())) {}
    ...
    private:
        // The libc++ implementation in Xcode 16 requires bitmaps stored in STL containers
        // to be copyable. We clone the underlying C struct to keep copy semantics explicit
        // and avoid sharing ownership across wrappers.
        static mtmd_bitmap *clone(const mtmd_bitmap *source)
        {
            if (!source)
            {
                return nullptr;
            ...
            const size_t n_bytes = mtmd_bitmap_get_n_bytes(source);
            const unsigned char *raw = mtmd_bitmap_get_data(source);

            if (mtmd_bitmap_is_audio(source))
            {
                const size_t n_samples = n_bytes / sizeof(float);
                std::vector<unsigned char> buffer(n_bytes);
                if (n_bytes > 0)
                {
                    std::memcpy(buffer.data(), raw, n_bytes);
                }
                mtmd_bitmap *copy = mtmd_bitmap_init_from_audio(
                    n_samples,
                    reinterpret_cast<const float *>(buffer.data()));
                apply_id(copy, mtmd_bitmap_get_id(source));
                return copy;
            }

            const uint32_t nx = mtmd_bitmap_get_nx(source);
            const uint32_t ny = mtmd_bitmap_get_ny(source);
            std::vector<unsigned char> buffer(n_bytes);
            if (n_bytes > 0)
            {
                std::memcpy(buffer.data(), raw, n_bytes);
            }
            mtmd_bitmap *copy = mtmd_bitmap_init(nx, ny, buffer.data());
            apply_id(copy, mtmd_bitmap_get_id(source));
            return copy;
        }

        static void apply_id(mtmd_bitmap *target, const char *id)
        {
            if (!target || !id || id[0] == '\0')
            {
                return;
            }
            mtmd_bitmap_set_id(target, id);
        }
    };
