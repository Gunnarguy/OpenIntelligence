...
### Highlights
- **Rust-Backed Tokenizer Engine:** Replaced the legacy pure-Swift `BertTokenizer` with a high-performance Rust-backed `swift-tokenizers` (DePasqualeOrg) wrapper target. This yields a 100x speedup in document tokenization alongside exact byte-level character offset mappings for citations. Renamed the SPM wrapper library to `TransformersTokenizers` and converted the tokenizer resource directories to utilize the `.bundle` extension. This prevents Xcode folder-group flattening and namespace copy collisions, resolving runtime initialization errors on devices.
...
- **RAG Refinements & Model Constraints**: RAG verification gate logic is refined to prevent false-positive refusals by ignoring query-specific terms and performing fuzzy plural/singular word mappings. Standard and reliability modes are updated to properly respect ungrounded fallback preferences, preventing unnecessary response discarding when ungrounded output is allowed. Model preference selection is hardened to strictly route execution on-device and cap RAG context packing budgets (6K tokens) when the 3B Core or 20B Advanced models are selected, completely bypassing Private Cloud Compute boundaries. All Swift 6 compiler warnings and actor isolation errors are resolved, the Developer Diagnostics Quick Sanity Check is adjusted to prevent false similarity failures across CoreML and legacy Natural Language embedding providers, a direct external link row to the Notion Roadmap database is added to the About Screen to easily track technical milestones, the iOS Silicon HUD status overlay is shifted horizontally to the right of the threads sidebar button to avoid overlapping, and the 'New Chat' action is updated to allocate a new active thread rather than deleting the previous conversation from disk, isolating active threads per container to prevent workspace bleed.
