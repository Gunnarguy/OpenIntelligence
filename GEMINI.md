...
9. **Re-verify suspicious symbols**: If a previous artifact claims a relationship between symbols (e.g., `RAGEngine`, `VectorDatabase.insert`), actively verify those symbols exist with code search before propagating them to new documentation. (That example is deliberately suspicious: `VectorDatabase` declares no `insert`.) <!-- verify-doc-claims: ignore -->
