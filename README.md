...
| **PCC**                    | Private Cloud Compute — Apple's confidential computing cloud. When on-device Neural Engine can't handle the request (too long, too complex), `AppleFoundationLLMService` routes to PCC. Your data is encrypted end-to-end, processed in a Secure Enclave with no persistent storage, and cryptographically attested (you can verify Apple isn't lying). Data deleted immediately after response. Zero-retention, zero-trust architecture.                                                                        |
