...
## 4.6 - 2026-07-14
### Fixed
- **[Orchestration]** The Silicon HUD legend is now reliably draggable: put it anywhere on screen and the position persists across launches. The whole legend rectangle is touch-targetable (working around a translucent-material hit-testing quirk on iOS 27), the drag activates on touch-down at highest priority, and an interfering double-tap recognizer was removed. The legend's default spot now sits fully below the navigation bar, whose touch region silently swallowed grabs on the legend's upper half.
