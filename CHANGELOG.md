...
## 4.6 - 2026-07-14
### Fixed
- **[Orchestration]** The Silicon HUD legend is now reliably draggable: put it anywhere on screen and the position persists across launches. The whole legend rectangle is touch-targetable (working around a translucent-material hit-testing quirk on iOS 27), the drag activates on touch-down at highest priority, and an interfering double-tap recognizer was removed. Drag movement resolves atomically (no more layer-tearing or spring-lag while dragging), and on iOS the legend now lives in its own floating layer above all app chrome — it is touchable and draggable at every position on screen — the floating layer claims exactly the legend's reported frame and passes every other touch through to the app.
