...
## 4.6 - 2026-07-14
### Fixed
- **[Orchestration]** The Silicon HUD legend is now reliably draggable: put it anywhere on screen and the position persists across launches. The whole legend rectangle is touch-targetable (working around a translucent-material hit-testing quirk on iOS 27), the drag activates on touch-down at highest priority, and an interfering double-tap recognizer was removed. Drag movement resolves atomically (no more layer-tearing or spring-lag while dragging), and the legend can be parked anywhere on screen — an invisible grab halo around the box guarantees it can always be re-grabbed, even flush against the status bar.
