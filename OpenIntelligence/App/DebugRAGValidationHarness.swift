    ...
    enum DebugRAGValidationHarness {
        /// `--rag-validation-stream-probe` hands the query a stream handler and reports what a chat
        /// view would have seen: when answer text started and stopped arriving, how many end markers
        /// came, and how long the answer took to finish after its text stopped.
        ///
        /// Without a handler the harness streamed nothing, so it could not see the gap the chat shows
        /// as a blinking cursor over a finished answer, and could not tell whether Deep Think or
        /// Maximum leaked intermediate text. Timing log lines inside the pipeline measure stages; this
        /// measures the interval the person waits through.
        @MainActor
        private final class StreamProbe {
            let start = Date()
            private(set) var firstText: TimeInterval?
            private(set) var lastText: TimeInterval?
            private(set) var firstFinal: TimeInterval?
            private(set) var textEvents = 0
            private(set) var finalEvents = 0
            private(set) var streamed = ""
            /// Called once, at the first end marker: the moment a chat would show a finished text.
            var onFirstFinal: (@MainActor () -> Void)?
            ...
            func record(_ event: LLMStreamEvent) {
                ...
                if event.isFinal {
                    ...
                    textEvents += 1
                    streamed += event.text
                    if firstText == nil { firstText = elapsed }
                    lastText = elapsed
                }
            }

            func summary(finishedAt done: TimeInterval, finalAnswer: String) -> String {
                func seconds(_ value: TimeInterval?) -> String {
                    value.map { String(format: "%.2fs", $0) } ?? "none"
                }
                let afterLastText = lastText.map { String(format: "%.2fs", done - $0) } ?? "n/a"
                let streamedText = streamed.trimmingCharacters(in: .whitespacesAndNewlines)
                let answer = finalAnswer.trimmingCharacters(in: .whitespacesAndNewlines)
                // How much of the finished answer the stream had already shown, by shared prefix.
                let sharedPrefix = zip(streamedText, answer).prefix { $0 == $1 }.count
                return "[StreamProbe] firstText=\(seconds(firstText)) lastText=\(seconds(lastText)) "
                    + "firstFinal=\(seconds(firstFinal)) done=\(String(format: "%.2fs", done)) "
                    + "afterLastText=\(afterLastText) textEvents=\(textEvents) finals=\(finalEvents) "
                    + "streamedChars=\(streamedText.count) answerChars=\(answer.count) sharedPrefix=\(sharedPrefix)"
            }
        }
        ...
        static var isVisualModeEnabled: Bool {
            cachedConfiguration != nil
                && (LaunchArguments.has("--rag-validation-visual") || LaunchArguments.has("rag-validation-visual"))
        }
        ...
        private static func seedEmbeddingProviderIfNeeded() {
            guard
                let raw = LaunchArguments.valueEither(for: "rag-validation-embedding-provider")?
        ...
        static func run(
            ...
            do {
                ...
                if configuration.shouldIngest, !configuration.inputURLs.isEmpty {
                    Log.info(
                        "[RAGValidation] Ingesting \(configuration.inputURLs.count) document(s)", category: .ingestion)
                ...
                let streamProbe: StreamProbe? =
                    LaunchArguments.has("--rag-validation-stream-probe")
                        || LaunchArguments.has("rag-validation-stream-probe")
                    ? StreamProbe() : nil
                // `--rag-validation-finish-after <seconds>` presses the chat's Stop that many seconds
                // after the text is complete, by calling `finishAnswerNow()` as `ChatScreen` does. It
                // exercises the path a person takes during "Checking sources…" or "Refining…",
                // which the sample library rarely holds open long enough to press by hand.
                if let streamProbe,
                    let raw = LaunchArguments.valueEither(for: "rag-validation-finish-after"),
                    let delay = Double(raw), delay >= 0
                {
                    streamProbe.onFirstFinal = {
                        Task { @MainActor in
                            try? await Task.sleep(for: .seconds(delay))
                            Log.info(
                                "[StreamProbe] finishAnswerNow() at \(String(format: "%.2f", Date().timeIntervalSince(streamProbe.start)))s",
                                category: .pipeline
                            )
                ...
                let probeHandler: LLMStreamHandler? = streamProbe.map { probe in
                    { @Sendable event in await MainActor.run { probe.record(event) } }
                }
                let (response, auditSnapshot) = try await ragService.queryWithAudit(
                    configuration.query,
                    topK: configuration.retrievalTopK ?? 3,
                    containerId: containerId,
                    streamHandler: probeHandler,
                    trace: trace
                )
                if let streamProbe {
                    let done = Date().timeIntervalSince(streamProbe.start)
                    // Stream events are delivered on tasks of their own, so let the last ones land.
                    try? await Task.sleep(for: .milliseconds(200))
                    Log.info(
                        streamProbe.summary(finishedAt: done, finalAnswer: response.generatedResponse),
                        category: .pipeline
                    )
                }
        ...
        private static func buildReport(
            ...
            lines.append(
                "Quality Mode: \(response.metadata.qualityModeName ?? configuration.qualityMode?.displayName ?? "Unknown")"
            )
            ...
            if response.retrievedChunks.isEmpty {
                ...
                for chunk in response.retrievedChunks.prefix(12) {
                    ...
                    lines.append(
                        "- rank=\(chunk.rank) sim=\(String(format: "%.3f", chunk.similarityScore)) page=\(page) source=\(sourceDocument.isEmpty ? "Unknown" : sourceDocument)"
                    )
            ...
            lines.append(
                "Report: \(configuration.outputDirectory.appendingPathComponent("rag_validation_report.txt").path)")
            if let copiedTraceURL {
        ...
        private static func copyTraceLog(into outputDirectory: URL) -> URL? {
            ...
            do {
                ...
                Log.warning(
                    "[RAGValidation] Failed to copy trace log: \(error.localizedDescription)", category: .pipeline)
                return nil
            }
        }
        ...
        private static func makeConfiguration() -> Configuration? {
            guard
                LaunchArguments.has("--rag-validation") || LaunchArguments.has("rag-validation")
                    || LaunchArguments.has("--rag-validation-visual") || LaunchArguments.has("rag-validation-visual")
            else {
                return nil
            }
            ...
            guard
                let query = LaunchArguments.valueEither(for: "rag-validation-query")?.trimmingCharacters(
            ...
            let shouldIngest =
                !(LaunchArguments.has("--rag-validation-skip-ingest")
                || LaunchArguments.has("rag-validation-skip-ingest"))
            let pccConsent = parsePCCConsent(LaunchArguments.valueEither(for: "rag-validation-pcc-consent"))
            let benchmarkEntitlement = parseBenchmarkEntitlement(
                LaunchArguments.valueEither(for: "rag-validation-entitlement"))
        ...
        private static func seedBenchmarkEntitlementIfNeeded(_ tier: WorkspaceTier?) {
            ...
            if tier == .free {
                ...
                defaults.set(
                    LegacyProtectionState.historicalPaidPurchase.rawValue, forKey: "entitlement.legacyProtectionState")
            }
            defaults.synchronize()
        ...
        private static func parseInputURLs() -> [URL] {
            let explicitValue =
                LaunchArguments.valueEither(for: "rag-validation-file")
