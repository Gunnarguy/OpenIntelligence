...
//
    ...
    class CameraManager: NSObject, ObservableObject {
        ...
        ///
        /// These were declared for `analyzeFrame`, which has **zero call sites**. The live delegate
        /// path is `performFrameAnalysis`, which never consulted them, so every delivered frame ran a
        /// full eight-request Vision pass synchronously on `analysisQueue`. The throttle below is the
        /// same idea, moved to the path that actually runs.
        private var lastAnalysisTime: Date = .distantPast
        private let analysisInterval: TimeInterval = 0.1  // 10 FPS max for analysis

        /// Throttle state for the live delegate path.
        ///
        /// `nonisolated(unsafe)` and read/written **only** from `captureOutput`, which AVFoundation
        /// always calls on `analysisQueue`, a serial queue. That serialisation is the synchronisation;
        /// there is no second accessor. The `@MainActor` `lastAnalysisTime` above cannot be used here
        ...
        /// Keeps the video buffers upright.
        ///
        /// Without this, nothing in this app ever told AVFoundation or Vision which way is up: there
        /// was no `videoRotationAngle` on any connection and no `CGImagePropertyOrientation` on any
        /// request. Buffers arrive in sensor orientation, which for a portrait-held iPhone is rotated
        /// a quarter turn, so Vision's normalized X ran down the screen and its Y ran across it. Every
        /// bounding box drawn from those coordinates was transposed, and text recognition, which
        /// tolerates skew but not a 90-degree rotation, mostly returned nothing.
        ///
        /// `AVCaptureDevice.RotationCoordinator` (iOS 17+) reports the angle that makes preview and
        /// capture horizon-level relative to gravity, and delivers updates on the main queue.
        ...
        /// as the device turns.
        ///
        /// This is what makes the sample buffers handed to Vision upright, which in turn makes
        /// Vision's normalized coordinates mean what the overlay assumes they mean. Setting it on
        /// the connection is preferable to passing a `CGImagePropertyOrientation` per request,
        /// because the buffers themselves arrive correct and every consumer benefits, including the
        /// still-capture path and anything added later.
        ///
        /// The coordinator is created with a nil preview layer, so
        /// `videoRotationAngleForHorizonLevelPreview` would return 0 and is deliberately not used;
        /// the preview layer sets its own connection's angle from its own coordinator. Only the
        /// capture angle matters here.
        private func startCoordinatingRotation(
            ...
            let apply: (CGFloat) -> Void = { [weak output] angle in
                guard let connection = output?.connection(with: .video) else { return }
                // `isVideoRotationAngleSupported` is not decoration: only 0, 90, 180 and 270 are
                // ever supported, and setting an unsupported angle raises.
            ...
            // The coordinator delivers KVO updates on the main queue, which is where this class
            // lives, so the observation can touch the connection directly.
        ...
        private func configureSession() {
            ...
            if session.canAddOutput(videoOutput) {
                ...
                startCoordinatingRotation(for: videoDevice, output: videoOutput)
            }
            ...
            if session.canAddOutput(photoOutput) {
                    ...
                    : videoDevice.activeFormat.supportedMaxPhotoDimensions.first
                {
                    ...
                    Log.info(
                        "[CameraManager] Photo dimensions set to \(maxDimensions.width)x\(maxDimensions.height)",
                        category: .ingestion)
                }
            }
            ...
            if #available(iOS 18.0, *) {
                ...
                VisionOCRThrottle.performSync {
                    do {
                        try requestHandler.perform([
                            textRequest, documentRequest, barcodeRequest, classifyRequest, animalRequest, faceRequest,
                            humanRequest, humanPoseRequest, animalPoseRequest, aestheticsRequest,
                        ])
                    } catch {
                        Log.debug(
                            "[CameraManager] Frame analysis failed: \(error.localizedDescription)", category: .ingestion
                        )
                    }
                }
            } else {
                ...
                VisionOCRThrottle.performSync {
                    do {
                        try requestHandler.perform([
                            textRequest, documentRequest, barcodeRequest, classifyRequest, animalRequest, faceRequest,
                            humanRequest, humanPoseRequest, animalPoseRequest,
                        ])
                    } catch {
                        Log.debug(
                            "[CameraManager] Frame analysis failed: \(error.localizedDescription)", category: .ingestion
            ...
            if isDocumentCapture {
                ...
                VisionOCRThrottle.performSync {
                    do {
                        ...
                        Log.error(
                            "[CameraManager] Capture analysis failed: \(error.localizedDescription)",
                            category: .ingestion)
                    }
                }
            } else {
    ...
    extension CameraManager: AVCaptureVideoDataOutputSampleBufferDelegate {
        nonisolated func captureOutput(
            ...
            // Throttle. `performFrameAnalysis` runs eight Vision requests synchronously and then
            // drains the GPU and sleeps, on this serial queue, so an unthrottled call here occupies
            // the queue for 100-300 ms per frame and AVFoundation silently drops everything behind
            // it. The dropped frames were invisible because there is no `didDrop` implementation.
            //
            // `analyzeFrame` had this guard and was never called. This is the same 10 FPS ceiling
            // on the path that runs. `CACurrentMediaTime` rather than `Date` because it is
            // monotonic and cannot jump backwards when the wall clock changes.
            let now = CACurrentMediaTime()
            guard now - lastLiveAnalysis >= 0.1 else { return }
            lastLiveAnalysis = now

            // The empty `Task { @MainActor in await MainActor.run { } }` that used to sit here did
        ...
        private nonisolated func performFrameAnalysis(ciImage: CIImage) {
            ...
            let frameAnalysis = FrameAnalysis(
                ...
                // The size Vision actually saw. The overlay needs it to undo the preview layer's
    ...
    extension CameraManager {
        ...
        nonisolated static func createHumanPose(from observation: VNHumanBodyPoseObservation) -> DetectedPose? {
            ...
            for jointName in jointNames {
                ...
                {  // Lower threshold to capture more joints
                    joints.append(
                        PoseJoint(
