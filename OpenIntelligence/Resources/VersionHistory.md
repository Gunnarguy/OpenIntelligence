...
- **Text from a PDF page is no longer shuffled into the wrong order.** Lines from a scanned or photographed page were being grouped by their position down the page using a lookup whose result depended on the order the page happened to arrive in. Adjacent lines came out swapped, so a paragraph read as a jumble of half-sentences even though every word had been read correctly. The same page could also come out differently on two runs. Lines are now matched to the nearest one deterministically, so a page reads in the order it was written.
...
- **Libraries you already have keep the index they were built with.** Nothing will prompt you to rebuild, because on a library created before this release the app has no way to tell the difference. Re-importing a document rebuilds it with the corrected model, and anything you import from now on is built correctly.
