...
3. Update each doc:
   - `CHANGELOG.md`: technical bullets under the section the preflight names in `documentation_targets.changelog_section` (not always `[Unreleased]`; AGENTS.md rule 18), each prefixed with its architectural tag (e.g., `[Retrieval] Added RRF weight override`).
